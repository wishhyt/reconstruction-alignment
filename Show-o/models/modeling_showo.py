...
class Showo(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        if type == 'default':
            ...
            if labels is not None:
                ...
                return logits, loss_t2i, None, loss_mmu
    ...
    def recon_loss(
        ...
        assert False, 'recon_loss is deprecated, use forward with type=recon instead'
        logits = self.showo(input_ids=input_ids, attention_mask=attention_mask)['logits']

        if labels is not None:
            # import pdb; pdb.set_trace()
            loss_recon = F.cross_entropy(
                logits[:batch_size_recon, -1025: -1].contiguous().view(-1, self.output_size),
                labels[:batch_size_recon, -1025: -1].contiguous().view(-1), ignore_index=-100,
            )
    ...
    def get_recon_predictions(self, logits, batch_size_recon=3):
        pred_logits = logits[:batch_size_recon, -1025: -1].contiguous().view(-1, self.output_size)
            ...
            if uncond_input_ids is not None and guidance_scale >= 0:
                ...
                if images_embeddings is not None:
                    ...
                    uncond_input_ids = torch.cat(
                        [uncond_input_ids[:, :-1025], input_ids[:, -1025:]], dim=1)
                    uncond_logits = self(uncond_input_ids, attention_mask=attention_mask[1])
                    
                    text_embeddings = self.showo.model.embed_tokens(input_ids)
                    part1 = text_embeddings[:, :2 , :]
                    part2 = text_embeddings[:, 2 :, :]
                    input_embeddings = torch.cat((part1, images_embeddings, part2), dim=1)
                    cond_logits = self(input_embeddings=input_embeddings, attention_mask=attention_mask[0])
                    # RuntimeError: The size of tensor a (1641) must match the size of tensor b (1733) at non-singleton dimension 1
                    # 把 cond_logits 和 uncond_logits 的维度对齐 （用 0）
                    cond_logits = torch.cat([torch.zeros((uncond_logits.shape[0], uncond_logits.shape[1] - cond_logits.shape[1], uncond_logits.shape[2]), device=uncond_logits.device), cond_logits], dim=1)
                else:
                    uncond_input_ids = torch.cat(
                ...
                # assert images_embeddings is not None
