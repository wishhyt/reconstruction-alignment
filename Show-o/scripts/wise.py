...
if __name__ == '__main__':
    ...
    json_list = ['../Benchmark/wise/data/natural_science.json', 
                 '../Benchmark/wise/data/cultural_common_sense.json',
    ...
    l = config.get("l", 0)
    r = config.get("r", 1000)
    for index, metadata in enumerate(dataset):
        if int(index) < l or int(index) >= r:
            continue 
               
        set_seed()
        prompt = metadata['Prompt']
        batch_size = 1
        print(f"Prompt ({index}/{1000}): '{prompt}'")
        os.makedirs(config.get("outdir"), exist_ok=True)
        out_path = os.path.join(config.get("outdir"), f"{metadata['prompt_id']}.png")
