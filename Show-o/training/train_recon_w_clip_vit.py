...
from training.prompting_utils import UniversalPrompting, create_attention_mask_predict_next, create_attention_mask_for_mmu_vit_recon, create_attention_mask_for_mmu_vit
...
def main():
    ...
    @torch.no_grad()
    def prepare_inputs_and_labels_recon(
            pixel_values_or_image_ids: Union[torch.FloatTensor, torch.LongTensor],
            t2i_pixel_values_or_image_ids: Union[torch.FloatTensor, torch.LongTensor],
            texts: Union[str, str],
            min_masking_rate: float = 0.0,
            is_train: bool = True,
            # is_recon: bool = False,
        ...
        images_feat = vision_tower(pixel_values_or_image_ids.clone().detach())
        
        image_tokens = vq_model.get_code(t2i_pixel_values_or_image_ids)
        image_tokens = image_tokens + len(uni_prompting.text_tokenizer)
        
        t2i_input_ids, t2i_labels, ____, mask_prob = mask_or_random_replace_tokens(
            image_tokens,
            mask_id,
            config,
            mask_schedule=mask_schedule,
        ...
        t2i_input_ids, t2i_masks, t2i_labels = uni_prompting((texts, t2i_input_ids, t2i_labels), 't2i_clip_recon')
        
        if hasattr(model, 'module'):
            images_embeddings = model.module.mm_projector(images_feat)
            text_embeddings = model.module.showo.model.embed_tokens(t2i_input_ids)
        else:
            images_embeddings = model.mm_projector(images_feat)
            
            text_embeddings = model.showo.model.embed_tokens(t2i_input_ids)
            
        part1_embeddings = text_embeddings[:, :2, :]
        part2_embeddings = text_embeddings[:, 2:, :]
        t2i_input_ids = torch.cat(
            (t2i_input_ids[:, :2], torch.ones_like(images_embeddings[:, :, 0]).long() * uni_prompting.ignore_id, t2i_input_ids[:, 2:]), dim=1
        )

        input_embeddings = torch.cat((part1_embeddings, images_embeddings, part2_embeddings), dim=1)
        part1_labels = t2i_labels[:, :2]
        part2_labels = t2i_labels[:, 2:]
        labels = torch.cat(
            (part1_labels, torch.ones_like(images_embeddings[:, :, 0]).long() * uni_prompting.ignore_id, part2_labels), dim=1
        )
        
        return input_embeddings, labels, t2i_input_ids
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for batch, batch_idx, dataloader_idx in combined_dataloader:
            ...
            # print('t2i : ', batch["t2i_flow"]["images"].shape, ', recon : ', batch["recon_flow"]["images"].shape, ', mmu : ', batch["mmu_flow"]["images"].shape)
            # *-------*-------*-------*-------*-------*-------*-------*-------*-------*-------*-------*
            ...
            pixel_values_recon_336 = torch.nn.functional.interpolate(pixel_values_recon, size=(336, 336), mode='bilinear', align_corners=False)

            pixel_values_recon = pixel_values_recon.to(accelerator.device, non_blocking=True)
            pixel_values_recon_336 = pixel_values_recon_336.to(accelerator.device, non_blocking=True)
            ...
            if config.training.recon_coeff > 0:
                (
                    input_embedding_recon,
                    labels_recon,
                    input_ids_recon,
                ) = prepare_inputs_and_labels_recon(pixel_values_recon_336, pixel_values_recon, [f"USER: \n { random.choice(recon_prompt_list) } ASSISTANT:" for _ in range(pixel_values_recon.shape[0])], config.training.min_masking_rate)
                
                attention_mask_recon = create_attention_mask_for_mmu_vit_recon(input_embedding_recon,
                                                                               input_ids_recon,
                                                                    return_inverse_mask=True,
                                                                    pad_id=int(uni_prompting.sptids_dict['<|pad|>']),
            ...
            with accelerator.accumulate(model):
                ...
                if config.training.recon_coeff > 0:
                    loss_recon = model(
                        ...
                        input_embeddings=input_embedding_recon,
