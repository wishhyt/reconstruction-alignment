...
class UniversalPrompting():
    ...
    def t2i_clip_recon_prompt(self, text_ids, image_ids, labels):
        ...
        for i in range(len(text_ids)):
            ...
            temp_ids = [int(self.sptids_dict['<|mmu|>']), int(self.sptids_dict['<|soi|>']), int(self.sptids_dict['<|eoi|>'])] + text_ids[i] + [self.text_tokenizer.eos_token_id]

            pad_length = 128
            if pad_length >= len(temp_ids):
                temp_ids = temp_ids + [self.pad_id] * (pad_length - len(temp_ids))
            else:
                assert False
                # should add the eos token
                temp_ids = temp_ids[:pad_length - 1] + [self.text_tokenizer.eos_token_id]
        ...
        return torch.cat(sequence_ids, dim=0), None, torch.cat(label_ids, dim=0)

    def t2i_gen_prompt(self, text_ids, image_ids, is_mmu=False):
        ...
        for i in range(len(text_ids)):
            ...
            temp_ids = [int(self.sptids_dict['<|t2i|>' if not is_mmu else '<|mmu|>'])] + text_ids[i] + [self.text_tokenizer.eos_token_id]
            if self.max_text_len >= len(temp_ids):
                temp_ids = [self.pad_id] * (self.max_text_len - len(temp_ids)) + temp_ids
    ...
    def t2i_recon_prompt(self, text_ids, image_ids, given_image_ids, labels):
        ...
        for i in range(len(text_ids)):
            if len(text_ids[i]) == 0:
                ...
                text_ids[i] = [self.text_tokenizer.bos_token_id] + text_ids[i]
            # note that, llama3 tokenizer automatically add the bot token at first but without eot
            temp_ids_ = text_ids[i] + [self.text_tokenizer.eos_token_id]
            pad_ids = [self.pad_id] * (self.max_text_len - len(temp_ids_) - 1)
            
            
            temp_ids = torch.cat([
                torch.tensor(pad_ids).to(device),  # pad tokens to fill the max length
                self.sptids_dict['<|mmu|>'].to(device),  # task token 
                self.sptids_dict['<|soi|>'].to(device),  # start of input for the text
                given_image_ids[i],
                self.sptids_dict['<|eoi|>'].to(device),  # eoi for the given image, this is optional
                torch.tensor(temp_ids_).to(device),
                self.sptids_dict['<|soi|>'].to(device),
                image_ids[i],
                self.sptids_dict['<|eoi|>'].to(device)
            ], dim=0)
            ...
            temp_label_ids = torch.cat([
                (torch.ones_like(torch.tensor(pad_ids)) * self.ignore_id).to(device),
                torch.tensor([self.ignore_id]).to(device),
                torch.tensor([self.ignore_id]).to(device),
                torch.ones_like(given_image_ids[i]) * self.ignore_id,  # for the given image, we do not predict
                torch.tensor([self.ignore_id]).to(device),
                (torch.ones_like(torch.tensor(temp_ids_)) * self.ignore_id).to(device),  # for the image tokens, we do not predict
                torch.tensor([self.ignore_id]).to(device),
                labels[i],  # the actual label for the text tokens
                torch.tensor([self.ignore_id]).to(device),
            ], dim=0)

            sequence_ids.append(temp_ids.unsqueeze(0))
            label_ids.append(temp_label_ids.unsqueeze(0))  # for the loss calculation, we need to predict the text tokens

        return torch.cat(sequence_ids, dim=0), None, torch.cat(label_ids, dim=0)


    def t2i_recon_gen_prompt(self, text_ids, image_ids, given_image_ids):
        ...
        for i in range(len(text_ids)):
            ...
            pad_ids = [self.pad_id] * (self.max_text_len - len(temp_ids) - 1)
            ...
            temp_ids = torch.cat([
                ...
                self.sptids_dict['<|mmu|>'].to(device),  # task token 
                self.sptids_dict['<|soi|>'].to(device),  # start of input for the text
                torch.tensor(given_image_ids[i]).to(device),
                self.sptids_dict['<|eoi|>'].to(device),  # eoi for the given image, this is optional
                torch.tensor(temp_ids).to(device),
                self.sptids_dict['<|soi|>'].to(device),
                image_ids[i],
    ...
    def t2i_recon_uncond_gen_prompt(self, text_ids, image_ids, delta=1026):
        ...
        for i in range(len(text_ids)):
            ...
            temp_ids = text_ids[i] + [self.text_tokenizer.eos_token_id]
            if self.max_text_len >= len(temp_ids):
                temp_ids = [self.pad_id] * (delta + self.max_text_len - len(temp_ids) - 1) + temp_ids
                temp_masks = [0] * (delta + self.max_text_len - len(temp_ids) - 1) + [1] * len(temp_ids)
            else:
                assert False
                temp_ids = temp_ids[:self.max_text_len - 1] + [self.text_tokenizer.eos_token_id]
                temp_masks = [1] * len(temp_ids)  # +2 for two special tokens
            ...
            # prompting -- [task token] [sot] [text tokens] [eot] [soi] [image tokens] [eoi]
            temp_ids = torch.cat([
                self.sptids_dict['<|t2i|>'].to(device),  # task token 
                torch.tensor(temp_ids).to(device),
                self.sptids_dict['<|soi|>'].to(device),
                image_ids[i],
                self.sptids_dict['<|eoi|>'].to(device)
    ...
    def __call__(self, input, task, padding=True, config=None):
        ...
        if task == "t2i":
            ...
            # print('[debug] t2i_prompt: ', sequence_ids_with_masks)
        ...
        elif task == "t2i_gen_mmu":
            text_ids = self.text_tokenizer(input[0])['input_ids']  # (B, max_len)
            image_ids = input[1]  # (B, #tokens)
            sequence_ids_with_masks = self.t2i_gen_prompt(text_ids, image_ids, is_mmu=True)
            
        elif task == "t2i_recon":
            text_ids = self.text_tokenizer(input[0])['input_ids']  # (B, max_len)
            image_ids = input[1]  # (B, #tokens)
            sequence_ids_with_masks = self.t2i_recon_prompt(
                text_ids, 
                image_ids, 
                input[2],  # given_image_ids
                input[3] if len(input) > 3 else None  # labels for the text tokens
            )
            
            # print('[debug] t2i_recon_prompt: ', sequence_ids_with_masks)
            
            
        elif task == "t2i_clip_recon":
            text_ids = self.text_tokenizer(input[0])['input_ids']  # (B, max_len)
            image_ids = input[1]  # (B, #tokens)
            sequence_ids_with_masks = self.t2i_clip_recon_prompt(text_ids, image_ids, input[2])
            
        elif task == "t2i_recon_gen":
            text_ids = self.text_tokenizer(input[0])['input_ids']  # (B, max_len)
            image_ids = input[1]  # (B, #tokens)
            given_image_ids = input[2]
            sequence_ids_with_masks = self.t2i_recon_gen_prompt(text_ids, image_ids, given_image_ids)
            
        elif task == "t2i_recon_gen_uncond":
            text_ids = self.text_tokenizer(input[0])['input_ids']
            image_ids = input[1]  # (B, #tokens)
            sequence_ids_with_masks = self.t2i_recon_uncond_gen_prompt(text_ids, image_ids)
            
        elif task == "t2i_recon_gen_uncond_clip":
            text_ids = self.text_tokenizer(input[0])['input_ids']
            image_ids = input[1]  # (B, #tokens)
            sequence_ids_with_masks = self.t2i_recon_uncond_gen_prompt(text_ids, image_ids, delta=576+2)
    ...
    if rm_pad_in_image:
        ...
        for i in range(mask_text_image_bi.shape[0]):
            ...
            if len(pad_end_idx[0]) != 0:
                ...
                mask_text_image_bi[i][sid_img[i]:, :pad_end_idx + 1] = 0
...
def create_attention_mask_predict_next_recon(sequence, pad_id=128256, soi_id=128257, eoi_id=128258, rm_pad_in_image=False,
                                       return_inverse_mask=True):
    # sequence is expected to be of shape [N, L]
    N, L = sequence.shape
    ...
    if rm_pad_in_image:
        for i in range(mask_text_image_bi.shape[0]):
            sid_img, eid_img = torch.where(sequence[i] == soi_id)[0][0], torch.where(sequence[i] == eoi_id)[0][0]
            pad_end_idx = torch.where(sequence[i] == pad_id)
            if len(pad_end_idx[0]) != 0:
                pad_end_idx = pad_end_idx[0][-1]
                mask_text[i][pad_end_idx + 1:, :pad_end_idx + 1] = 0
                mask_text_image_bi[i][sid_img:, :pad_end_idx + 1] = 0
                mask_text_image_bi[i][sid_img: eid_img + 1, eid_img + 1:] = 0
            else:
                assert False
    else:
        assert False
    ...
    mask_debug = mask_text.clone().cpu().numpy()
    mask_debug = mask_debug.reshape(N, L, L).astype('uint8') * 255
    cv2.imwrite('mask_debug.bmp', mask_debug[0])  # save the first mask for debugging, remove in production
...
def create_attention_mask_for_mmu_vit_recon(
        sequence,
        input_ids,
        return_inverse_mask=True,
        system_prompt_len=0,
    ...
    # causal_mask[:, :, :, :index] = 1
    causal_mask[:, :, :, 1+system_prompt_len+1:index] = 1
    causal_mask[:, :, -1 - resolution : -1, -1 - resolution : -1] = 1
    ...
    mask_debug = causal_mask.clone().cpu().numpy()
    mask_debug = mask_debug.reshape(N, L, L).astype('uint8') * 255
    if return_inverse_mask:
        inverted_mask = 1.0 - causal_mask.type(torch.int64)
        inverted_mask = inverted_mask.masked_fill(
